        ...
        <div className='text-black flex flex-col  bg-gray-100'>
            ...
            <div className='mt-56 sc-xs:mt-32 sc-md:mt-8 flex flex-col w-full justify-center py-m px-6  max-w-screen-sc-2xl mx-auto'>
                ...
                {projectList.map((projectArr: any, index: number) => (
                    /*<div key={index} className='py-m grid grid-cols-1 sc-sm:grid-cols-2 sc-lg:grid-cols-4 justify-between gap-xl'>
                        {projectArr.map((projectData: any, innerIndex: number) => (
                            <ProjectCard2 key={innerIndex} data={projectData} activeToken={activeToken} len={projectArr.length}/>
                        ))}
                    </div>*/

                    <div key={index} className='py-m grid grid-cols-1 sc-sm:grid-cols-2 sc-xl:grid-cols-4 justify-between gap-xl'>
                        {projectArr.map((projectData: any, innerIndex: number) => (
                            <ProjectCardMini key={innerIndex} data={projectData} activeToken={activeToken}
