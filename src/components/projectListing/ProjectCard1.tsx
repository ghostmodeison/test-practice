...
export default function ProjectCard1({ data, activeToken }: any) {
    ...
    return (
        <div className="flex flex-col gap-4 p-6 bg-white rounded-2xl justify-between border border-[#e6e6e6] shadow-lg">
            {/* Header Section */}
            <div className="flex flex-col sc-md:flex-row gap-7">
                {/* Project Image */}
                ...
                <div className="flex flex-col gap-2 flex-1">
                    ...
                    <h2 className="text-2xl text-[#064a1c] font-instrument">{data.name.length > 50 ? toCapitalizedCase(data.name.slice(0, 50) + "..") : toCapitalizedCase(data.name)}</h2>
                    <p className="text-base text-neutral-1200">{data.details.length > 100 ? data.details.slice(0, 100) + "..." : data.details}</p>
                </div>
            </div>

            <div>
                {/* Info Boxes */}
                <div className="flex flex-col sc-sm:flex-row sc-sm:gap-6">
                    <InfoBox label="ESTIMATED CREDITS" value={data.estimation_annual_estimated_reductions}
                             unit="tCO2e"/>
                ...
                <div className="w-full border rounded-2xl p-4 mt-2 relative">
                    <p className="text-xs font-semibold uppercase absolute left-[16px] top-[-6px] px-3 text-[10px] text-[#064a1c]">ESG
                        Goals</p>
