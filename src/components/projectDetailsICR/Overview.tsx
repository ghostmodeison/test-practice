...
const getCountryByCode = (code: string): string => {
    const country = (Object.keys(countries) as Array<keyof typeof countries>).find(
        key => countries[key] === code.toUpperCase()
    );
...
const Overview = (props: any) => {
    ...
    //     router.replace(Routes.BuyCredit);
    // }
    console.log(props);
    const countryCode = props.data.projects[0].rawData.countryCode;
    const url = `https://flagsapi.com/${countryCode}/flat/64.png`;

    return (
        <div className='w-full  flex-col pb-xl relative'>
            <div id="header" className=' flex justify-between items-center'>
                <div className='text-black text-f-3xl font-semibold'>{props.data.projects[0].rawData.fullName.toUpperCase()}</div>
                <div className='flex flex-row'>
                    <div id="ck_verified" className='flex flex-row items-center px-m py-s bg-positiveSubtitle rounded-3xl'>
                        ...
                        <span className='ml-s  text-black font-semibold'>ENVR Verified</span>
                    </div>
            ...
            <img src={`${props.data.projects[0].rawData.thumbnail}`} className='bg-cover my-l min-h-[300px] w-full max-h-[400px] bg-gray-500' />
            <div id="follow" className='flex justify-end  items-center '>
                <div className='text-white font-semibold text-f-m py-s px-l bg-brand1-500 rounded-lg'>
                    {props.data.projects[0].rawData.status.toUpperCase()}
                </div>
            ...
            <div className='w-[488px] h-[236px] p-xl bg-brand3 absolute bottom-0 text-white left-3xl rounded-2xl flex flex-col ' >
                <div className='pb-l border-b border-neutral-100 text-f-m flex-1'>
                    {props.data.projects[0].rawData.description ? props.data.projects[0].rawData.description.length > 250 ? props.data.projects[0].rawData.description.slice(0, 250) + "..." : props.data.projects[0].rawData.description : "No details available"}
                </div>
                <div className='flex flex-row mt-l justify-between'>
                    <div id="ck_verified" className='flex flex-row items-center px-m py-s '>
                        ...
                        <span className='ml-s  text-white font-semibold'>{getCountryByCode(props.data.projects[0].rawData.countryCode)}</span>
                    </div>
                    {props.data.projects[0].rawData.description && props.data.projects[0].rawData.description.length > 250 &&
