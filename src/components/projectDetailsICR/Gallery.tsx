    ...
    return (
        <div className="flex-1 bg-white mt-xl rounded-xl">
            ...
            <div className='p-xl text-black flex justify-evenly'>
                <div className='w-full h-[422px] grid grid-cols-3 gap-xl overflow-scroll hide-scrollbar'>
                    {galleryData.map((image: any, index: any) => (
                        <GalleryDiv key={index} image={image} name={image} />
                    ))}
                </div>
            </div>
...
const GalleryDiv = (props: any) => {
    return (
        <div className='flex bg-neutral-100 flex-1 justify-center items-center h-[192px]'>
            <img src={props.image} alt={props.name} className='bg-cover w-full h-full' loading="lazy" />
        </div>
    );
};