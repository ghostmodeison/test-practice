...
const FloatingBar: React.FC<FloatingBarProps> = ({ scrollToSection, activeSection, openModal, activeToken }) => {
    return (
        <div className="fixed bottom-6 left-0 right-0 z-50  flex justify-end mx-xl space-x-2 items-end">
            {activeSection !== "footer" &&
                <div className='flex-1 bg-gray-900 px-4 py-2 shadow-lg flex justify-evenly rounded-[150px]'>
                    <button
                        className={`text-white text-f-s px-4 py-2 rounded-[100px] text-base hover:bg-green-500 transition-all `}
            ...
            {activeToken && <button className=' bg-brand1-500 w-7xl h-7xl flex justify-center items-center rounded-full hover:bg-brand1-400 shadow-sm shadow-brand1-200' onClick={openModal}>
